...
<style>
    ...
    .butaca { position: absolute; cursor: grab; user-select: none; }
    ...
    .grid-cell { border: 1px dashed rgba(139,92,246,0.2); box-sizing: border-box; }
    ...
    .model-btn:hover:not(.model-btn--active) { background: var(--bg-raised); color: var(--text); }
            ...
            <div class="admin-card">
                ...
                <div style="display:flex;flex-wrap:wrap;align-items:center;gap:12px;margin-bottom:16px;">
                    <div style="display:flex;align-items:center;gap:8px;">
                        <label class="admin-label" style="margin:0;">Modelo:</label>
                        <div style="display:flex;border:1px solid var(--border);border-radius:8px;overflow:hidden;">
                            <button type="button" class="model-btn model-btn--active" data-model="standard">Estándar (20×12)</button>
                            <button type="button" class="model-btn" data-model="large">Grande (60×36)</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="admin-canvas-wrap">
                    <div id="canvas" class="grid-container"
                         style="background:var(--bg-raised);border:1px solid var(--border);border-radius:12px;overflow:hidden;">
                    </div>
                </div>
            </div>
...
<script>
    ...
    /* ── Modelos de plano ── */
    const MODELS = {
        standard: { CELL: 50, COLS: 20, FILAS: 12, SW: 44, SH: 48, SO: 3 },
        large:    { CELL: 25, COLS: 60, FILAS: 36, SW: 21, SH: 23, SO: 2 }
    };

    let CELL, COLS, FILAS, SW, SH, SO;
    let modo = 'add';
    let seatCount = 1;
    let history = [];
    ...
    /* ── Inicializar canvas según modelo ── */
    function initCanvas(modelKey) {
        const m = MODELS[modelKey];
        CELL = m.CELL; COLS = m.COLS; FILAS = m.FILAS;
        SW = m.SW; SH = m.SH; SO = m.SO;
        ...
        canvas.style.height = `${FILAS * CELL}px`;
        canvas.style.gridTemplateColumns = `repeat(${COLS}, ${CELL}px)`;
        canvas.style.gridAutoRows = `${CELL}px`;
    ...
    document.querySelectorAll('.model-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            if (btn.classList.contains('model-btn--active')) return;
            const hasSeat = canvas.querySelector('.butaca');
            if (hasSeat && !confirm('Cambiar de modelo borrará todos los asientos colocados. ¿Continuar?')) return;
            document.querySelectorAll('.model-btn').forEach(b => b.classList.remove('model-btn--active'));
            btn.classList.add('model-btn--active');
            initCanvas(btn.dataset.model);
        });
    });
    ...
    function seatSVG(zona) {
        ...
        return `<svg viewBox="0 0 44 48" width="${SW}" height="${SH}" xmlns="http://www.w3.org/2000/svg">
    ...
    function createSeat(zona, ejeX, ejeY) {
        ...
        div.style.left = `${ejeX * CELL + SO}px`;
        div.style.top  = `${ejeY * CELL + SO}px`;
    ...
    function makeDraggable(el) {
        ...
        document.addEventListener('mousemove', e => {
            ...
            el.style.left = `${ejeX * CELL + SO}px`;
            el.style.top  = `${ejeY * CELL + SO}px`;
        ...
        document.addEventListener('mouseup', () => {
            ...
            if (occupiedCells.has(newKey)) {
                ...
                el.style.left = `${origX * CELL + SO}px`;
                el.style.top  = `${origY * CELL + SO}px`;
            } else {
    ...
    function deshacerUltimaAccion() {
        ...
        if (last.tipo === 'add') {
            ...
            last.element.style.left = `${last.oldX * CELL + SO}px`;
            last.element.style.top  = `${last.oldY * CELL + SO}px`;
